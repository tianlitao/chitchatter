...
export const UserInfo = ({ userId }: UserInfoProps) => {
  ...
  const updateCustomUsername = () => {
    ...
    if (trimmedUsername.length) {
      showAlert(t('usernameChangedTo', { username: trimmedUsername }), {
        severity: 'success',
      })
    } else {
      showAlert(t('usernameReset'), { severity: 'success' })
    }
  }
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        <FormControl sx={{ width: '100%' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            />
            <Tooltip title={t('revealUserInfo')}>
              <IconButton
                sx={{
                  ml: 1.5,
          ...
          <FormHelperText>{t('yourUsername')}</FormHelperText>
        </FormControl>
      </form>
      <Dialog open={isInfoDialogOpen} onClose={handleInfoDialogClose}>
        ...
        <DialogContent>
          <DialogContentText>{t('publicKeyInfo')}</DialogContentText>
          <PublicKey publicKey={publicKey} />
          <DialogContentText>{t('privateKeyInfo')}</DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleInfoDialogClose}>{t('close')}</Button>
        </DialogActions>
      </Dialog>
    </>
  )
